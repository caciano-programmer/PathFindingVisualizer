...
const comments = [
  ...
  `
    /*
        Priority Queue used for A* and Dijkstra's algorithm, not neccessary in both cases,
        but a very useful optimization since there is the constant need to select the node with smallest cost
    */
  `,
];

export const highlightTypes = { keywords, separators, functions, comments };

export const codeText = `
    type SinglePath = { length: number; path: number[] };
    type Path = Map<number, SinglePath>;
    type Explored = { paths: Path; visited: number[] };
